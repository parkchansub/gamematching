 ...
 * [주의] 시나리오 2의 차이점 안내
 *
 * 시나리오 2에는 게임을 일부러 지려는 어뷰저가 존재한다. 어뷰저는 상대보다 높은 실력을 갖고 있음에도 일부러 지는 유저이다.
 * 어뷰저는 자신보다 낮은 실력을 가진 상대에게만 80%의 확률로 지며, 게임에 걸리는 시간은 10분 이하이다.
 * 나머지 20% 확률에서는 어뷰저가 승리하며, 이때 걸리는 시간은 일반 게임과 같은 방식으로 산출되며 10분을 초과할 수 있다.
 * 전체 유저 중 어뷰저의 비율은 5%이다.
 * 게임에 걸리는 시간은 3 ~ 10 사이의 랜덤한 값이다.
 * 자신보다 높은 실력을 가진 유저에게는 위와 같이 일부러 지려는 행동을 하지 않는다.
 * 다시 말해, 어뷰저끼리 게임을 하게 된다면, 둘 중 높은 실력을 가진 어뷰저가 80% 확률로 진다.
 *
 * 점수
 ...
 *
 * 정확성 점수 1은 시뮬레이션이 끝난 뒤, 응시자가 유저에게 부여한 등급으로 정렬한 결과와 유저 고유 실력으로 정렬한 결과가 얼마나 유사한지를 평가한다.
 * 두 결과가 유사할수록 점수가 높다.
 * 유저 A, B, C의 고유 실력 순위가 각각 1, 2, 3위이고, 부여된 등급 순위가 각각 1, 3, 2위일 경우,
 * 정확성 점수 1은 U = 32 * 0.5 = 4.5이므로, ((4.5 - (0 + 1 + 1)) / 4.5) * 100 로 계산된다.
 * 고유 실력 순위 = 유저가 처음부터 갖고 있는 고유한 실력 값을 기준으로 내림차순 정렬했을 때 순위를 의미한다.
 * 가장 실력 값이 높은 유저가 1위이다.
 * 부여된 등급 순위 = 응시자가 조정한 유저의 grade를 기준으로 내림차순 정렬했을 때 순위를 의미한다.
 * 가장 grade가 높은 유저가 1위가 된다.
 * 만약 grade값이 같은 유저가 여러 명이라면, 중복 값들에 대해서 동일 순위로 표시하고, 중복 순위 다음 값에 대해서는 중복 개수만큼 떨어진 순위를 부여한다.
 * 예를 들어 유저 4명의 grade값이 [100, 99, 99, 95] 라면 각각 [1위, 2위, 2위, 4위]가 된다.
 * 정확성 점수 2는 두 유저가 매칭될 때마다 얼마나 비슷한 실력의 유저끼리 매칭되었는지를 평가한다.
 * 매칭된 두 유저의 실력 차이에 2를 곱한 뒤 평균을 구해 점수에서 차감한다. 즉, 실력이 비슷한 유저끼리 매칭될수록 점수가 높다.
 * 효율성 점수는 매칭 신청 후 매칭되기까지 걸린 시간의 합을 점수에서 차감한다. 즉, 유저가 기다린 시간이 짧을수록 점수가 높다.
 * 최종 점수는 (시나리오 1에서 획득한 총점) x (시나리오 1의 배점 가중치) + (시나리오 2에서 획득한 총점) x (시나리오 2의 배점 가중치)로 부여된다.
 * 단, 배점 가중치는 지원자에게 공개하지 않는다.
 *
 * 참고
 ...
 *
 * API REFERENCE
 * 540분 동안 시뮬레이션을 진행한다. 시뮬레이션은 분 단위로 진행되며, 매 분을 한 턴으로 가정한다. 예를 들어, 0분을 0턴, 1분을 1턴, ... , 540분을 540턴으로 가정한다.
 * 0턴은 시뮬레이션을 시작하는 턴으로, 매칭을 신청하는 유저가 존재하지 않는다. 1턴부터 매칭 대기열이 활성화된다.
 *
 * 대기 중인 유저를 짝지어주는 Match API와, 대기 중인 유저들의 정보를 받을 수 있는 WaitingLine API, 이번 턴에 끝난 게임 결과 정보를 받을 수 있는 GameResult API가 주어진다.
 * 게임 결과 정보를 참고하여 ChangeGrade API를 이용해 특정 유저의 등급을 수정할 수 있다.
 * Match API는 대기열에서 대기 중인 유저들을 매칭시켜 줄 수 있으며, 동시에 다음 턴으로 넘어간다(1분 진행).
 * 따라서 시뮬레이션의 진행을 위해서는 매칭시킬 유저가 없더라도 Match API를 이용해 아무도 매칭시키지 않음을 전달해야 한다.
 *
 * 1턴부터 540턴까지는 매칭을 신청할 수 있는 기간이다. 541턴 이후로는 새로운 매칭 신청이 발생하지 않는다.
 * 540턴에 매칭을 신청한 유저의 경우 최대 555턴까지 매칭을 기다릴 수 있으며, 556턴이 되면 매칭을 취소한다.
 * 만약 555턴에 매치가 성사되어 40분 동안 게임이 진행된다면 595턴에 게임 결과를 받아볼 수 있다. 따라서 595턴까지 GameResult API를 이용해 게임 결과를 확인해야 한다.
 *
 * 따라서 1턴~555턴까지는 대기열에 있는 유저들을 확인하는 WaitingLine API와 대기열의 유저를 매치시키는 Match API를 사용할 수 있다.
 * 그 외 턴에서 WaitingLine API는 빈 배열만을 반환하며 Match API는 어떤 유저도 매칭시키지 못한다.
 * 이번 턴에 끝난 게임 결과를 반환하는 GameResult API와, 유저의 등급을 수정할 수 있는 ChangeGrade API는 1턴~595턴 중 어느 턴에든 실행할 수 있다.
 * 595턴이 끝나야 시뮬레이션이 종료되기 때문에, Match API를 이용해 매치시킬 유저가 없더라도 595턴까지 진행해야 한다.
 * 마지막으로, 595턴에서 한번 더 Match API를 실행시키면 서버 상태가 finished가 되며, 어떤 API도 사용할 수 없다.
 * 즉, 유저의 등급을 변화시키는 ChangeGrade API는 595턴이 끝나기 전에 실행해야 한다.
 *
